		...
		if (search && search.value) {
			...
			sortedArticles = sortedArticles.filter((v) => {
				return v.metadata.title.toLowerCase().includes(search.value.toLowerCase()) ||
					   v.metadata.category.toLowerCase().includes(search.value.toLowerCase());
			});
...
<svelte:head>
	<title>Jb's Tutorials</title>
</svelte:head>

<div class="flex flex-row flex-wrap items-end justify-between gap-4">
	<div>
		<h1 class="title headercolor text-2xl leading-snug">Tutorials</h1>
	</div>
